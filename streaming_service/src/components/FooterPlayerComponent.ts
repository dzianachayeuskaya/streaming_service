...
export class FooterPlayerComponent extends SmartComponent {
  private intermediateValue: number = -0.5;
  public likeBtn: HTMLElement;
  private shaffleBtn: HTMLElement;
  private skipbackBtn: HTMLElement;
  ...
  private activeValueRange: HTMLElement;
  private valueRange: HTMLElement;
  private valueBtn: HTMLElement;
  ...
  constructor(
    ...
    private isSongListSectionEmpty: boolean,
    public data: IPlayerData
  ) {
    super(data);

    this.likeBtn = el('button.player__track__like');
    this.shaffleBtn = el('button.player__shaffle-btn');
    this.skipbackBtn = el('button.player__skipback-btn');
    this.playBtn = el('button.player__play-btn');
    this.skipnextBtn = el('button.player__skipnext-btn');
    this.repeatBtn = el('button.player__repeat-btn');

    this.startTime = el('span.player__time-start');
    this.activeRange = el('.player__range-play__active');
    this.range = el(
      '.player__range-play',
      { id: 'range-play' },
      this.activeRange
    ...
    this.activeValueRange = el('.player__range-value__active');
    this.valueRange = el(
      '.player__range-value',
      { id: 'range-value' },
      this.activeValueRange
    );
    this.valueBtn = el('button.player__value__btn');
  }

  protected repairHandlers() {
    this.likeBtn.addEventListener('click', this.handleLike);
    this.playBtn.addEventListener('click', this.onPlayBtnClick);
    this.range.addEventListener('pointerdown', this.handlePointerDownOnRange);
    if (this.audioEl) {
      this.audioEl.addEventListener('timeupdate', this.onProgress);
      this.audioEl.addEventListener('ended', this.onTrackEnded);
    }

    document.addEventListener('keydown', this.onKeydown);

    this.shaffleBtn.addEventListener('click', this.onShaffleBtnClick);
    this.repeatBtn.addEventListener('click', this.onRepeatBtnClick);
    this.skipnextBtn.addEventListener('click', this.onNextBtnClick);
    this.skipbackBtn.addEventListener('click', this.onBackBtnClick);
    this.valueBtn.addEventListener('click', this.onValueClick);
    this.valueRange.addEventListener(
      'pointerdown',
      this.handlePointerDownOnValueRange
    );
  }

  protected getTemplate({
    song: { image, artist, name, path, duration, likes },
    listPosition,
    mix,
    repeat,
    isPlaying,
    ...
    const gainValue = localStorage.getItem('value');
    this.gain.gain.value = gainValue ? +gainValue : 0;
    ...
    const trackName = el('.player__track-name flex', [
      el('img.player__track__img', {
        src: image,
        alt: `${name} - ${artist.name}`,
      }),
      el('.player__track-name__content', [
        el('.flex player__name__header', [
          el('h3.player__track__h3', name),
          this.likeBtn,
        ]),
        el('p.player__track__author', artist.name),
      ]),
    ]);

    this.shaffleBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 16 16" fill="none"
  ...
  <path
    ...
    this.shaffleBtn.classList.toggle('shaffle-btn--active', mix);

    if (mix) this.determineSkipBtnActivity(ListPositionEnum.None);
    else this.determineSkipBtnActivity(listPosition);

    if (this.isSongListSectionEmpty) {
      this.shaffleBtn.setAttribute('disabled', 'disabled');
      this.determineSkipBtnActivity(ListPositionEnum.Single);
    }

    this.skipbackBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 16 16" fill="none"
xmlns="http://www.w3.org/2000/svg">
<path
  ...
  fill="#AAAAAA" />
    ...
    this.playBtn.innerHTML = `<svg width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
<path class="player__play-el"
...
<path
  ...
  fill="white" />
    ...
    this.playBtn.classList.toggle('play-btn--active', isPlaying);

    this.skipnextBtn.innerHTML = `<svg width="10" height="12" viewBox="0 0 10 12" fill="none"
xmlns="http://www.w3.org/2000/svg">
<path
  ...
  fill="#AAAAAA" />
    ...
    this.repeatBtn.innerHTML = `<svg width="14" height="12" viewBox="0 0 14 12" fill="none"
xmlns="http://www.w3.org/2000/svg">
<path
...
<path
  ...
  fill="#AAAAAA" />
    ...
    const controls = el(
      ...
      el('.player__controls__footer', [
        this.startTime,
        this.audioEl,
        this.range,
        el('.player__time-end', convertMsToMinutesAndSeconds(duration)),
      ])
    );

    this.valueBtn.innerHTML = `<svg width="12" height="14" viewBox="0 0 12 14" fill="none"
  xmlns="http://www.w3.org/2000/svg">
  <path d="M4 9.5H1C0.867392 9.5 0.740215 9.44732 0.646447 9.35355C0.552678 9.25979 0.5 9.13261 0.5 9V5C0.5 4.86739 0.552678 4.74021 0.646447 4.64645C0.740215 4.55268 0.867392 4.5 1 4.5H4L8.5 1V13L4 9.5Z"
  ...
  <path d="M10.9124 5.58582C11.0981 5.77153 11.2454 5.99201 11.3459 6.23466C11.4464 6.47731 11.4981 6.73739 11.4981 7.00003C11.4981 7.26267 11.4464 7.52274 11.3459 7.7654C11.2454 8.00805 11.0981 8.22853 10.9124 8.41424"
    stroke="#AAAAAA" stroke-linecap="round" stroke-linejoin="round" />
</svg>`;

    this.fillValueRange();

    const value = el('.player__value', [this.valueBtn, this.valueRange]);

    const player = el('.player flex', [trackName, controls, value]);
    const footer = el('footer.footer', player);

    return footer;
  }

  private determineSkipBtnActivity(listPosition: ListPositionEnum) {
    switch (listPosition) {
      case ListPositionEnum.First:
        this.skipbackBtn.setAttribute('disabled', 'disabled');
        this.skipnextBtn.removeAttribute('disabled');
        break;
      case ListPositionEnum.Last:
        this.skipnextBtn.setAttribute('disabled', 'disabled');
        this.skipbackBtn.removeAttribute('disabled');
        break;
      case ListPositionEnum.Single: {
        this.skipbackBtn.setAttribute('disabled', 'disabled');
        this.skipnextBtn.setAttribute('disabled', 'disabled');
        break;
      }
  ...
  private determineTrackCurrentTime = (e: MouseEvent) => {
    const trackTime =
      ((e.offsetX / this.range.offsetWidth) * this.data.song.duration) / 1000;

    if (this.audioEl) this.audioEl.currentTime = trackTime;
  ...
  private handlePointerDownOnRange = (e: MouseEvent) => {
    this.determineTrackCurrentTime(e);

    this.range.addEventListener('pointermove', this.determineTrackCurrentTime);
    document.addEventListener('pointerup', (e) => {
      this.range.removeEventListener(
        'pointermove',
        this.determineTrackCurrentTime
      );
    });
  };

  private determineCurrentVolume = (e: MouseEvent) => {
    const value = e.offsetX / this.valueRange.offsetWidth - 1;
  ...
  private handlePointerDownOnValueRange = (e: MouseEvent) => {
    this.determineCurrentVolume(e);

    this.valueRange.addEventListener(
      'pointermove',
      this.determineCurrentVolume
    );
    document.addEventListener('pointerup', (e) => {
      this.valueRange.removeEventListener(
        'pointermove',
        this.determineCurrentVolume
      );
    });
  ...
  private getCurrentRange(): string {
    const increment = 10 / this.data.song.duration;
    return Math.min(increment * this.data.currentTime * 10000, 100) + '%';
  }
  ...
  private setMixState() {
    ...
    this.data = { ...this.data, mix: !this.data.mix };
  }

  private setRepeatState() {
    localStorage.setItem(
      'mode',
      JSON.stringify({
        mix: this.data.mix,
    ...
    this.data = { ...this.data, repeat: !this.data.repeat };
  }

  private changePlayBtnState() {
    this.playBtn.classList.toggle('play-btn--active', this.data.isPlaying);
  }

  private changeShaffleBtnState(isActive: boolean) {
    this.shaffleBtn.classList.toggle('shaffle-btn--active', isActive);
  }

  private changeRepeatBtnState(isActive: boolean) {
    this.repeatBtn.classList.toggle('repeat-btn--active', isActive);
  }

  private handleLike = async () => {
    const sectionTrackEl = document.getElementById(`${this.data.song.id}`);
    const newStateSong = await this.model.clickOnLike(this.data.song.id);
    const newStateLike = newStateSong
      ? convertSongDataForTrackComponent(newStateSong).likes
      : this.data.song.likes;
    if (sectionTrackEl) {
      const likeBtn = sectionTrackEl.querySelector('button.track__like-btn');
      likeBtn?.classList.toggle('like-btn--active', newStateLike);
    }
    this.likeBtn.classList.toggle('like-btn--active', newStateLike);
  };

  private onShaffleBtnClick = () => {
    this.setMixState();
    this.changeShaffleBtnState(this.data.mix);
    if (this.data.mix) {
      this.determineSkipBtnActivity(ListPositionEnum.None);
    } else this.determineSkipBtnActivity(this.data.listPosition);
  };

  private onRepeatBtnClick = () => {
    this.setRepeatState();
    this.changeRepeatBtnState(this.data.repeat);
  };

  private onPlayBtnClick = async () => {
    if (this.audioEl && this.audioContext) {
      ...
      if (!this.data.isPlaying) {
        try {
          ...
          this.data = { ...this.data, isPlaying: !this.audioEl.paused };
          this.changePlayBtnState();
        } catch (err) {
          console.log(err);
        }
      } else {
        this.audioEl.pause();
        this.data = { ...this.data, isPlaying: !this.data.isPlaying };
        this.changePlayBtnState();
      }
    }
    // const source = this.audioContext.createBufferSource();

    // const audioBuffer = await getSongFile(this.data.song.filename);
    // if (audioBuffer) {
  ...
  private onProgress = (e: Event) => {
    if (e.target) {
      this.data = {
        ...this.data,
        currentTime: (e.target as HTMLMediaElement).currentTime,
      };
    }

    this.activeRange.style.width = this.getCurrentRange();
    this.startTime.textContent = convertMsToMinutesAndSeconds(
      this.data.currentTime * 1000
    );
  };

  private onValueClick = () => {
    if (this.gain && this.audioEl) {
      if (this.gain.gain.value > -1) {
        this.intermediateValue = this.gain.gain.value;
        this.gain.gain.value = -1;
      } else {
        this.gain.gain.value = this.intermediateValue;
      }
      setValue(this.gain.gain.value);
    }
    this.fillValueRange();
  };

  private fillValueRange = () => {
    if (this.gain)
      this.activeValueRange.style.width =
        (this.gain.gain.value + 1) * 100 + '%';
  };

  public updateDataAndComponent(
    newSongData: {
    ...
    if (newSongData) {
      const { trackData, listPosition } = newSongData;
      if (trackData) {
        this.updateData(
          convertSongDataForFooterPlayer(trackData, listPosition),
          true
        );
        this.model.setCurrentFooterSongId(trackData.id);
      }
    }
  }
  ...
  private onSkip(direction: 'Next' | 'Back') {
    this.pausePlayback();
    this.model
      .getTrackFromApi(this.playlistId, this.data.song.id, direction)
      .then((newSongData) => {
        this.updateDataAndComponent(newSongData);
        this.playBtn.click();
      });
  }

  private onSkipClick = (direction: 'Next' | 'Back') => this.onSkip(direction);
  ...
  private onTrackEnded = () => {
    ...
    if (this.data.repeat) {
      this.playBtn.click();
    } else this.onSkip('Next');
  };

  private onNextBtnClick = () => {
    this.onSkipClick('Next');
  };

  private onBackBtnClick = () => {
    this.onSkipClick('Back');
  };
    ...
    if (this.audioEl)
      switch (e.code) {
        case 'ArrowLeft':
          ...
          this.data = {
            ...this.data,
            currentTime: (this.data.currentTime -= 5),
          };
          break;
