...
export class TracksSection extends AbstractComponent {
  constructor(
    private list: TrackComponent[],
    private title: string
  ) {
    super(list);
  }

  protected getTemplate(list: TrackComponent[]) {
    const tracksSection = el(
      'section.tracks section tabs-content section--active',
      { 'data-target': 'tracks' }
    );
    const tracksHeader = el('.tracks__header flex');
    tracksHeader.innerHTML = `<div class="tracks__header__number">№</div>
...
<svg width="12" height="13" viewBox="0 0 12 13" fill="none"
    ...
    <path d="M11 1.5H1C0.723858 1.5 0.5 1.72386 0.5 2V12C0.5 12.2761 0.723858 12.5 1 12.5H11C11.2761 12.5 11.5 12.2761 11.5 12V2C11.5 1.72386 11.2761 1.5 11 1.5Z"
...
<div class="tracks__header__time"><svg width="14" height="14" viewBox="0 0 14 14" fill="none"
    ...
    const listEl = el(
      'ul.tracks__list',
      list.map((item) => item.getInteractiveElement())
    );
    const tracksContent = el(
      '.tracks__content',
      list.length
        ? [tracksHeader, listEl]
        : el(
            'p.tracks__content__header',
            'В текущем плейлисте нет ни одного трека'
          )
    );
    setChildren(tracksSection, [
      el('h2.tracks__h2 title__h2', this.title),
      tracksContent,
    ]);
    return tracksSection;
  }

  updateElement(data: TrackComponent[]): HTMLElement {
    this.data = data;
    this.removeElement();
