...
export class Aside extends AbstractComponent {
  private tracksLink: HTMLElement;
  private plLink: HTMLElement;

  constructor(
    private playlistsList: IPlaylist[],
    private currentPlId: string | null
  ) {
    super(playlistsList);
    this.tracksLink = el('a.aside__link aside__tabs-link', {
      href: `/`,
      'data-pl-id': 'null',
    });

    this.plLink = el('a.aside__link aside__tabs-link', {
      href: `/?playlistId=all`,
      'data-pl-id': 'all',
    });
  }

  protected getTemplate(plList: IPlaylist[]) {
    const openBtn = el('button.search__btn-open');
    openBtn.innerHTML = `<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
  <path
  ...
  <path d="M18.9999 19L15.5 15.5001" stroke="#AAAAAA" stroke-width="2" stroke-linecap="round"
    ...
    this.tracksLink.innerHTML = `<svg width="25" height="27" viewBox="0 0 25 27" fill="none" xmlns="http://www.w3.org/2000/svg">
<path
...
<span class="aside__link__text">Треки</span>
`;

    this.plLink.innerHTML = `<svg width="22" height="26" viewBox="0 0 22 26" fill="none" xmlns="http://www.w3.org/2000/svg">
<path
...
<span class="aside__link__text">Плейлисты</span>
`;

    const items = plList.map((item: IPlaylist) => {
      const link = el(
        'a.aside__link',
        { href: `/?playlistId=${item.id}`, 'data-pl-id': item.id },
        item.name
      );
      return el('li.aside__item', link);
    });

    const list = el('ul.aside__list', [
      el('li.aside__item', this.tracksLink),
      el('li.aside__item', this.plLink),
      items,
    ]);

    const activeEl = list.querySelector(
      'a[data-pl-id="' + this.currentPlId + '"]'
    );
    activeEl?.classList.add('aside__link-active');

    const nav = el('nav.aside__nav', [openBtn, list]);

    const aside = el('aside.aside', [
      el('h2.aside__h2 visually-hidden', 'Левая панель навигации'),
      nav,
    ]);
