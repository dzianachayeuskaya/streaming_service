...
export class UserAccount extends AbstractComponent {
  constructor({ imgSrc, username }: IUser) {
    super({
      imgSrc,
      username,
    });
  ...
  protected getTemplate({ imgSrc, username }: IUser) {
    const userSvg = el('');
    userSvg.innerHTML = `<svg class="header__user__svg" width="6" height="11" viewBox="0 0 6 11" fill="none"
    ...
    const userButton = el('button.header__user', [
      el('img.header__user__img', {
        src: imgSrc,
        alt: 'Изображение пользователя',
      }),
      el('span.header__user__text', username),
      userSvg,
    ]);
